...
b %>%
  ...
  geom_text(aes(label = porc), size = 2.5, hjust = -.1,
            fontface = "bold") +
  scale_fill_manual(values = c("khaki2", "salmon", "steelblue"),
                    labels = c("The Caribbean", "Central America", "South America")) +
  labs(x = "",
...
left_join(base, latam) %>% 
  ...
  labs(y = "Total additional amount divided\nby monthly minimum wage", x = "Coverage\n(direct beneficiaries divided by total population)",
...
left_join(base, latam) %>% 
  ...
  scale_color_brewer(type = "qual", palette = "Set1",
                     labels = c("Exp. horizontal","Exp. vertical","Programa nuevo")) +
  labs(y = "Total additional amount divided\nby monthly GDP per capita", x = "Coverage\n(direct beneficiaries divided by total population)",
       color = "Type of innovation") + 
  theme_minimal() +
  scale_fill_brewer(type = "qual", palette = "Set1") +
  guides(colour = guide_legend(override.aes = list(size = 4))) +
...
b %>% 
  ...
  labs(x = "\nProporción de población en el registro social",
       y = "Apoyos entregados por COVID-19 entre población\n",
...
b %>% 
  ...
  scale_y_continuous(label = percent_format(accuracy = 1)) +
  theme_minimal() +
  labs(x = "\nProportion of population in the social registry",
